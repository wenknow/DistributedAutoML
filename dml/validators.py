...
class BaseValidator(ABC):
    ...
    def cache_chain_metadata(self):
        ...
        for hotkey in self.bittensor_network.metagraph.hotkeys:
            if hotkey not in ["5GGtaQQ6RLjEV9idE48NNwe2qLFMRhEkqHXBAnTrhnwUxiQy","5EWdLxY75qAfKGc8Sd9cxqxHKr4wUmnYgcCGrfCFaum43m1M", "5Exps1XhjBCvkT11sFcuzbDpF9pHw4oa94RpbXYkJp2aUkW3"]:
                continue
            try:
        ...
        logging.info(f"Cached chain metadata for {len(self.chain_metadata_cache)} miners")
        

    def check_chain_submission(self, func, hotkey: str) -> Tuple[bool, Optional[int], Optional[str]]:
        """
        ...
        for uid, record in self.gene_record_manager.records.items():
            ...
            cached_signature = self.gene_record_manager.compute_function_signature(record['func'])
    ...
    def validate_and_score(self):
        ...
        for hotkey_address in self.bittensor_network.metagraph.hotkeys:
            ...
            if should_download:
                ...
                if computed_hash != chain_meta['hash']:
                    logging.warning(f"Chain hash mismatch for {hotkey_address} gene {gene[2]} expected {chain_meta['hash']} found {computed_hash}")
                    accuracy_scores[hotkey_address] = torch.zeros((len(self.config.Validator.dataset_names),), device=self.config.device)
                    continue
                
                try:
                    func_signature = self.gene_record_manager._compute_function_signature(compiled_func)
                    downloaded_genes[hotkey_address] = (gene, compiled_func, func_signature)
                    
                    if func_signature:
                        block_number = chain_meta['block_number']
                        if func_signature not in round_signatures:
                            round_signatures[func_signature] = (block_number, hotkey_address)
                        else:
                            existing_block, existing_hotkey = round_signatures[func_signature]
                            if block_number < existing_block:
                                # This submission is earlier, update the record
                                round_signatures[func_signature] = (block_number, hotkey_address)
                                logging.warning(f"Duplicate found. {existing_hotkey} is copying the gene used by {hotkey_address}")
                                accuracy_scores[existing_hotkey] = torch.zeros((len(self.config.Validator.dataset_names),), device=self.config.device)
                                continue

                            else: 
                                logging.warning(f"Duplicate found. {hotkey_address} receives 0 score")
                                accuracy_scores[hotkey_address] = torch.zeros((len(self.config.Validator.dataset_names),), device=self.config.device)
                                continue
                    else:
                        logging.warning(f"Failed to calculate the gene fingerprint {hotkey_address}")
                        accuracy_scores[hotkey_address] = torch.zeros((len(self.config.Validator.dataset_names),), device=self.config.device)
                        continue
                ...
                if existing_record.get('gene_string') is not None:
                    gene, func, gene_string = load_individual_from_json(data={"expression":existing_record['gene_string']}, pset=self.pset, toolbox=self.toolbox)
                    func_signature = self.gene_record_manager._compute_function_signature(func)
                    if func_signature:
                        if func_signature in round_signatures:
                            existing_block, existing_hotkey = round_signatures[func_signature]
                            if existing_record['block_number'] < existing_block:
                                round_signatures[func_signature] = (existing_record['block_number'], hotkey_address)
                                accuracy_scores[existing_hotkey] =  torch.zeros((len(self.config.Validator.dataset_names),), device=self.config.device)
                                logging.info(f"Existing record by {existing_hotkey} copying from {hotkey_address}. Fixing.")
                            else:
                                accuracy_scores[hotkey_address] = torch.zeros((len(self.config.Validator.dataset_names),), device=self.config.device)
                                logging.info(f"Existing record by {hotkey_address} is a duplicate ")
                            
                        else:
                            round_signatures[func_signature] = (existing_record['block_number'], hotkey_address)
                else:  
                    logging.info(f"{hotkey_address} is a cached failed gene. Assigning zero")
                    accuracy_scores[hotkey_address] = torch.zeros((len(self.config.Validator.dataset_names),), device=self.config.device)
        ...
        for hotkey_address in self.bittensor_network.metagraph.hotkeys:
            if hotkey_address not in accuracy_scores:  # Skip if already processed (cached or failed)
                ...
                if downloaded_data:
                    ...
                    if func_signature in round_signatures:
                        earliest_block, original_hotkey = round_signatures[func_signature]
                        if chain_meta['block_number'] > earliest_block and hotkey_address != original_hotkey:
                            logging.warning(f"Duplicate function from {hotkey_address}. Original at block {earliest_block} by {original_hotkey}")
                            accuracy_scores[hotkey_address] = torch.zeros((len(self.config.Validator.dataset_names),), device=self.config.device)
                        else:
